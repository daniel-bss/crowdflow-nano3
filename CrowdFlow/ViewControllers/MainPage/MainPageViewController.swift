...
extension MainPageViewController: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        switch pageSection {
        case .trainInformation(let trainInformations):
            let vc = TrainInformationViewController(id: indexPath.row, trainInformation: trainInformations[indexPath.row])
            self.navigationController?.pushViewController(vc, animated: true)
        case .promotions(_), .newsAndStories(_, _, _):
            let alert = UIAlertController(title: "Sorry...", message: "These \"Promotions\" and \"News\" are just for dummy data", preferredStyle: UIAlertController.Style.alert)
