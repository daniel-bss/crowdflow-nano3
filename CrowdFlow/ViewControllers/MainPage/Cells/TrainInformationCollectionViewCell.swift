...
class TrainInformationCollectionViewCell: UICollectionViewCell {
    ...
    public func configure(trainInformation: TrainInformationData) {
        destinationLabel.text = "To \(trainInformation.finalDestination)"
    ...
    private func setupCarriageView() {
        ...
        otherLabel.font = .poppinsRegular(size: 9.5)
    ...
    private func setupLowerView() {
        ...
        Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(timerDidChange), userInfo: nil, repeats: false)
